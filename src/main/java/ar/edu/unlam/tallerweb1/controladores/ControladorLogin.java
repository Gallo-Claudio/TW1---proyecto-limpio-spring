...
public class ControladorLogin {
	...
	public ModelAndView irALogin() {
		...
		return new ModelAndView("login", modelo); // En la vista (archivo.jsp) lo llamo con la key del map, en este caso
													// "usuario"
	}
...
////////////////////////////////// Agrgado en la clase de MVC del 02/05/2019 //////////////////////////////	
	...
	public ModelAndView verHolaMundo() { // *VERHOLAMUNDO - Nombre como llamo al metodo asociado al RequestMapping
		...
		return new ModelAndView("holamundo", modelo); // *HOLAMUNDO - Nombre de la vista (archivos con la extension
														// .jsp)

		// Los nombres del path, del metodo y de la vista, pueden llamarse de forma
		// distinta entre si. Pero se recomienda que tengan el mismo nombre para
		// mantener una coherencia y mayor claridad
...
////////////////////////////////////// Clase del 9/5/2019 //////////////////////////////////////////
	...
	/// Anotacion en la URL -
	/// localhost:8080/holis?nombre=VARAIBLE1&apellido=VARIABLE2
	@RequestMapping(path = "/holis", method = RequestMethod.GET) // Request mapping --->"action" (Url+Metodo Http) por
																	// default es Get, (si no se pone)
	public ModelAndView holis(@RequestParam(value = "nombre") String firstname,
			@RequestParam(value = "apellido") String secondname) { /// Metodo que devuelve la vista
		ModelMap modelo = new ModelMap(); // Coleccion Map (Key-Value) ---> Creo un modelo - A traves del Map viajan los
											// datos
		...
		return new ModelAndView("holis", modelo); // [holis] nombre de la vista - [modelo] nombre de la coleccion Map,
													// estamos pasando la informacion que esta en el MAp
	}
...
/////////////////////////////// Actions pasados por Agustina (con sus respectivas vistas) - Ejercicios del 9/5/2019 ///////////////////////////////////////	
	...
	public ModelAndView cambiarCaracteres(@PathVariable("caracteres") String caracter,
			@PathVariable("repeticion") Integer repeticion) {
		ModelMap model = new ModelMap();
	...
	public ModelAndView cambiar(@PathVariable("caracteres") String caracter,
			@PathVariable("repeticion") Integer repeticion) {
		ModelMap model = new ModelMap();
		List<String> lista = new ArrayList();
	...
	public ModelAndView saludar(@RequestParam(value = "nombre") String nombre,
			@RequestParam(value = "ap") String apellido) {
		ModelMap model = new ModelMap();
...
//ACTION 1
	...
	public ModelAndView presentarFormulario() {
		...
		Persona persona = new Persona(); // Instancio una persona para pasar un objeto vacio a traves del ModelMap a la
											// vista
		modelo.put("identidad", persona); // Agrego el objeto a la coleccion Map (ModelMap)

		return new ModelAndView("formulario", modelo); // voy a la vista formulario y paso el objeto persona a traves de
														// la instancia (modelo) del ModelMap
	}

	List<Persona> listadoPersonas = new ArrayList(); // defino una coleccion Array, donde guardar los datos recibidos de
														// la vista formulario
//ACTION 2

	@RequestMapping(path = "/formulario", method = RequestMethod.POST)
	public ModelAndView agregar(@ModelAttribute("usuarioPersona") Persona usuarioPersona, HttpServletRequest request) { // Recibo
																														// a
																														// traves
																														// de
																														// ModelAttribute
...
/////////////////// Mismo EJERCICIO que el anterior - Hecho Por Juli ////////////////////////////////////////

	List<Persona> misdatos = new ArrayList<Persona>();

	@RequestMapping("/pepe")
	public ModelAndView mostrarFormulario() {
		return new ModelAndView("Formulario2");
	}

	@RequestMapping(value = "/GuardarDatos", method = RequestMethod.POST)
	public ModelAndView datosFormulario(@ModelAttribute("Persona") Persona mip, HttpServletRequest request) {

		ModelMap modelo = new ModelMap();
		misdatos.add(mip);
		return new ModelAndView("Formulario2", modelo);
	}
